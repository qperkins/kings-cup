...
When REDIS_DIRECT_URL is set (production single-instance), bypass Sentinel
and connect directly to that Redis URL instead.
"""
from __future__ import annotations

import os

_REDIS_DIRECT_URL = os.environ.get("REDIS_DIRECT_URL")

if _REDIS_DIRECT_URL:
    import redis.asyncio as aioredis

    def get_master():
        """Direct Redis connection for reads/writes (single-instance production)."""
        return aioredis.from_url(_REDIS_DIRECT_URL, decode_responses=False)

    def get_replica():
        """Pub/sub on the same node when Sentinel is not in use."""
        return aioredis.from_url(_REDIS_DIRECT_URL, decode_responses=False)
else:
